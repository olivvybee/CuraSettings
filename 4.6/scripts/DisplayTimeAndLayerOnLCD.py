...
# Description:  This plugin displayes the current layer, total number of layers, and remaining time on the LCD of the printer
...
class DisplayTimeAndLayerOnLCD(Script):
    ...
    def getSettingDataString(self):
        return """{
            "name":"Display Remaining Time and Layer on LCD",
            "key":"DisplayRemainingTimeAndLayerOnLCD",
            "metadata": {},
            "version": 2,
            "settings":
            {
            ...
            for layer in data:
                ...
                for line in lines:
                    if line.startswith(";LAYER_COUNT:"):
                        max_layer = line
                        max_layer = int(max_layer.split(":")[1])

                        line_index = lines.index(line)
                        lines[line_index] = "M117 1/{} - {}".format(max_layer, total_time_string)
                    ...
                    if line.startswith(";TIME:"):
                        ...
                        total_time_string = "{:d}h{:02d}m".format(h, m)           # Now we put it into the string

                        if max_layer > 0 and layer_index <= max_layer:
                            lines[line_index] = "M117 {}/{} - {}".format(layer_index, max_layer, total_time_string)   # And print that string instead of the original one
                        ...
                        time_left = max(total_time - current_time, 0)   # Here we calculate remaining time
                        ...
                        current_time_string = "{:d}h{:2d}m".format(int(h1), int(m1))   # Here we create the string holding our time

                        if max_layer > 0 and layer_index <= max_layer:
                            lines[line_index] = "M117 {}/{} - {}".format(layer_index, max_layer, current_time_string)           # And now insert that into the GCODE
